...
public class CommonWebpageDAO extends IDAO<Webpage> {
     ...
     * @param outputStream 文件输出流
     */
    private ResultBundle<String> exportWebpageDATABASEyDomain(QueryBuilder queryBuilder, Boolean includeRaw) {
       return exportWebpageDATABASEyDomain(queryBuilder,
                searchResponse -> Lists.newLinkedList(),
                searchResponse -> {
                    List<String> resultList = Lists.newLinkedList();
                    List<Webpage> webpageList = warpHits2List(searchResponse.getHits());
     ...
     * @param outputStream 文件输出流
     */
    private ResultBundle<String>  exportWebpageDATABASE(QueryBuilder queryBuilder, Boolean includeRaw,String dataType,String dataCategory) {
      return  exportDataBase(queryBuilder,
    		  searchResponse -> Lists.newLinkedList(),
                searchResponse -> {
                    List<String> resultList = Lists.newLinkedList();
                    List<Webpage> webpageList = warpHits2List(searchResponse.getHits());
     ...
     * @param outputStream 文件输出流
     */
    public ResultBundle<String>  exportWebpageDATABASEySpiderUUID(String uuid, Boolean includeRaw,String dataType,String dataCategory) {	
       return exportWebpageDATABASE(QueryBuilders.matchQuery("spiderUUID", uuid).operator(Operator.AND), includeRaw,dataType,dataCategory);
    }
     ...
     * @param outputStream 文件输出流
     */
    public ResultBundle<String> exportWebpageDATABASEyDomain(String domain, Boolean includeRaw) {
    	return exportWebpageDATABASEyDomain(QueryBuilders.matchQuery("domain", domain).operator(Operator.AND), includeRaw);
    }
