...
public abstract class IDAO<T> {
     ...
     * @return 经过分词的输出流
     */
    protected ResultBundle<String>  exportDataBase(QueryBuilder queryBuilder,Function<SearchResponse, List<List<String>>> labelsSupplier,  Function<SearchResponse, List<String>> contentSupplier,String dataType,String dataCategory) {
    	final int size = 50;
        String scrollId = null;
        for (int page = 1; ; page++) {
            LOG.debug("正在输出{}第{}页", queryBuilder, page);
            ...
            for(int i=0;i<combine.size();i++){
            	String data= combine.get(i);
            	JSONObject js=JSONObject.parseObject(data);
            	User user=new User();
            	user.setTitle(js.getString("title"));
            	user.setTempType(js.getString("dataType"));
            	user.setCatagory(js.getString("dataCategory"));
            	user.setTempContent(js.getString("content"));
            	
            	String keywords=js.getString("keywords");
            	if("".equals(keywords)){
            		user.setTempKeyword("");
            	}else{
            		keywords=keywords.replace("[", "").replace("]", "").replace("\"", "");
            		user.setTempKeyword(keywords);
            	}
            	user.setSourceUrl(js.getString("url"));
            	if(StringUtils.isNotEmpty(js.getString("publishTime"))){
            		user.setReleaseDate(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(js.getLong("publishTime"))));
            	}
            	userdao.save(user);
            }
        }
     ...
     * @return 经过分词的输出流
     */
    protected ResultBundle<String> exportWebpageDATABASEyDomain(QueryBuilder queryBuilder, Function<SearchResponse, List<List<String>>> labelsSupplier, Function<SearchResponse, List<String>> contentSupplier) {
    	final int size = 50;
        String scrollId = null;
        for (int page = 1; ; page++) {
            LOG.debug("正在输出{}第{}页", queryBuilder, page);
